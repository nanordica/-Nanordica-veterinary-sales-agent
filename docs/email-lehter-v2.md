...
5. **Tagasiside otse kirjaga** (kõnesamm S5 ja K6 vahelt eemaldatud 28.07): näidis kohal (S5) → **+7 p → K6** tagasiside-küsimus. Tagasiside (S6) tuleb K6 (või K7/K8) vastusest. K6 vaikus → K7 (+7 p), K8 (+14 p) → Lost.
...
flowchart TD
    ...
    S5 -->|"+7 p"| K6["✉ K6 · tagasiside-küsimus"]
...
| **K6** tagasiside-küsimus | E5-B | vet | näidis kohal (S5) | **+7 p pärast tarnet** (eeldus — kinnita) | outreach-writer |
...
| — (sama staadium) | K6–K8 | delivered_at + 7 p → K6 | **UUS:** feedback_at (K6/K7/K8 vastusest), fb_emails_sent |
...
**Tagasiside-redel:** tarne tuvastamisel (delivered_at) ootab tick 7 p ja saadab K6; vaikuse korral K7 (+7 p) ja K8 (+14 p) → Lost. Tagasiside (feedback_at) tekib K6/K7/K8 vastusest, mille inbox-triage klassifitseerib. Meelise ainus samm lehtris on V1.
...
**Vajalikud muudatused koodis:** STATE_KEYS + uued field'id (shipped_at, tracking_barcode, pakomats_id, delivered_at, feedback_at, fb_emails_sent, meeting_at); tick'i sammud: omniva_track poll, K6 taimer (delivered_at + 7 p), K7/K8 redel; inbox-triage'i kategooriatesse "kohtumissoov"; K4 sõnastus klikkijale (kes pole veel "jah" öelnud); mail-mcp guardrail'id (≤1 kiri/24 h, max 5) kehtivad vetile — K5 (Verale) vajab sisemist erandit.
...
8. **K6 ajastus** pärast tarnet: +7 p (v1 loogika) — kinnita või muuda.
