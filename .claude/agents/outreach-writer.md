...
5. Never send email 6: if asked to follow up a deal with `emails_sent` = 5, refuse and report it. The /tick orchestrator owns the exhausted-ladder → Lost transition; do not move the deal yourself.
...
4. One CTA. Opt-out line. Run `language-checker`. Send. Update `discount_code`. No stage change — the deal stays in Engaged until sales-detector sees a purchase or sample redemption (there is no "Offer" stage in the pipeline).
